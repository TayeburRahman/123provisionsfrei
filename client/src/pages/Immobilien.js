    ...
    <section className="flex justify-center py-8 md:py-16 bg-gray-50">
      ...
      <div className="text-left w-11/12 lg:w-3/4">
        ...
        <div className="flex justify-center">
          <button disabled={search} className="bg-green-600 p-2 text-white rounded-b-3xl" onClick={(e) => filterData(e)}>{search ? 'Suche...' : 'Filter'}</button>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <Link className={`bg-green-600 p-2 text-white rounded ${noData ? 'opacity-0 pointer-events-none': ''} `} onClick={()=>setSearch(true)} to={`/immobilien/${+page+1}`}>{search ? 'Searching...' : `Weiter`}</Link>
        </div>
